...
let orbitControls;
let geekoMixer, geekoAction, characterController, geekoRigidBody, bugRigidBody, bugMesh, geekoCollider;
let geekoObject;
...
const init = async () => {
  ...
  const getValidBugPosition = () => {
    ...
    while (!validPos && attempts < 50) {
      ...
      const clearOfObstacles = obstacles.every(obs => 
        Math.sqrt((obs.x - x) ** 2 + (obs.z - z) ** 2) > cubeSize * 1.5
      );
      if (clearOfObstacles) {
        validPos = { x, y: 1, z };
      }
  ...
  // Bug Physics Body
  const bugBodyDesc = RAPIER.RigidBodyDesc.fixed()
    .setTranslation(bugPos.x, bugPos.y, bugPos.z);
  ...
  if (animObject.animations.length > 0) {
    ...
    const clip = animObject.animations.find(a => a.name === 'Walk') || animObject.animations[0];
  ...
  const animate = () => {
    ...
    const distance = Math.sqrt(
      Math.pow(bugPosition.x - newPos.x, 2) +
      Math.pow(bugPosition.z - newPos.z, 2)
    );
    
    if (distance < 1.0) { // Trigger score when close enough
      score();
